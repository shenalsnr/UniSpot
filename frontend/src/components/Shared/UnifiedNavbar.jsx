...
const UnifiedNavbar = ({ title = "UniSpot", moduleName, links = [], rightActions, centerModule = false }) => {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[oklch(48.8%_0.243_264.376)] bg-gradient-to-r from-white/80 via-transparent to-white/80 border-b border-white/20 shadow-xl px-4 md:px-8 py-2 md:py-3 flex justify-between items-center transition-all relative">
        ...
        links.length > 0 && (
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 space-x-6 lg:space-x-10 items-center">
            {links.map((link, idx) => {
              ...
              const linkClasses = `text-lg font-bold tracking-wide transition-all duration-300 relative group pb-1 ${
